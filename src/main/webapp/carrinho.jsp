...
<body onload="detectarTodosProdutos()">
	<% 
    ...
    <main id="main">
    	<%
			...
			produtos = token.pegarCarrinho(request);
    		Map<Fornecedor, Map<Produto, Integer>> resposta = produtoServico.dividirPorFonecedor(produtos);
    	%>
        <h1>Carrinho</h1>
        <%for (Entry<Fornecedor, Map<Produto, Integer>> entry : resposta.entrySet()) {
        	Fornecedor fornecedor = entry.getKey();
            DecimalFormat formatter = new DecimalFormat("#.00");
            float precoFinal = 0;

        %>
        <div class="pedido" id="pedido<%=fornecedor.getId()%>">
        	<%
        	for(Entry<Produto, Integer> key: entry.getValue().entrySet()) {
        			Produto produto = key.getKey();
        			int quantidade = key.getValue();
		            ...
		            if (produto.getDesconto() > 0.0){
		            	...
		            	precoFinal += (produto.getPreco() * (1-produto.getDesconto()))*quantidade;
		            }else{
		            		valor += formatter.format(produto.getPreco());
		            		precoFinal +=produto.getPreco() *quantidade;
		            }
	            
        		
        		%>
            <div class="produto">
                <img src="produtos/<%= produto.getNome().replaceAll(" ", "")%><%= produto.getId()%>.png" alt="" class="imagemProduto">
                <%if (produto.getDesconto() > 0){ %>
                <p class="nomeProduto"><%=produto.getNome() %><br>De: <%= valor%><br>Por: <%= valorFinal%></p>
                <%}else{ %>
                 <p class="nomeProduto"><%=produto.getNome() %><br><%=valor%></p>
                <%}; %>
                <p class="avaliacao">Curtidas: <%=produto.getCurtidas()%></p>
                <div class="quantidade">
                    <h2 class="contador"><%=quantidade %></h2>
                </div>
            </div>
            <%}; %>
         
            <div class="finalizarProduto">
                <div class="fornecedor">
                    <img src="fornecedor/<%=fornecedor.getId()+fornecedor.getNome() %>.png" alt="" class="logoForncedor">
                    <p class="nomeForncedor"><%=fornecedor.getNome() %></p>
                </div>
                <div class="formas" id="formas<%=fornecedor.getId()%>">
                    <label for="formaEntrega">Forma da entrega:</label>
                    <select name="formaEntrega" id="formaEntrega<%=fornecedor.getId()%>">
                        <option value="Entrega">Entrega</option>
                        <option value="Retirada">Retirada</option>
                    </select>
                    <br>
                    <label for="cartao">Cartao:</label>
                    <select name="cartao" id="cartao<%=fornecedor.getId()%>">
                    	<option value="Dinheiro">Dinheiro</option>
                    
                        <%
                        ArrayList<Cartao> cartoes = cartaoServico.listar(Integer.parseInt(token.pegarId(request)));
                        for (Cartao cartao: cartoes){
                        %>
                        	<option value="<%=cartao.getApelido()%>"><%=cartao.getApelido()%></option>
                        <%} %>
                    </select>
                </div>
                ...
                <%
                for(Entry<Produto, Integer> key: entry.getValue().entrySet()) { 
                String produto = key.getValue()+"x "+key.getKey().getNome();%>
                    <p><%=produto %></p>
                    <%}; %>
                    <p>Frete - R$ <%=formatter.format(entry.getKey().getPrecoEntrega()) %></p>   
                    <p class="valorFinal">Valor final - R$<%=formatter.format(precoFinal+fornecedor.getPrecoEntrega())%></p>
                </div>
            </div>

            <div class="finalizarCompra">
                <img src="files/delete.svg" alt="" onclick="apagarCompra('<%=fornecedor.getId()%>')">
                <button class="finalizarCompraButton" onclick="comprar('<%=fornecedor.getId()%>')">Finalizar Compra</button>
            </div>
